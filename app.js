...
// ══════════════════════════════════════════════════
// AUTH  — FIX: no top-level DOM queries, no currentUser at parse time
// ══════════════════════════════════════════════════
...
function doSignup() {
  ...
  const acres = (document.getElementById('su-acres')?.value || '1').trim();
  const crop  = (document.getElementById('su-crop')?.value  || 'Tomato');
  ...
  function showError(msg) {
    ...
    err.id = 'signup-error';
    err.style.cssText = 'background:#fee;color:#c00;border:1px solid #fcc;border-radius:8px;padding:10px 14px;font-size:13px;margin-bottom:12px;';
    err.textContent = msg;
    const btn = document.querySelector('#auth-signup .auth-btn');
    if (btn) btn.parentNode.insertBefore(err, btn);
  }

  if (!first || !last || !phone || !state) {
  ...
  currentUser = { name: first + ' ' + last, state, acres, crop };
  _launchApp();
...
function _launchApp() {
  ...
  if (metaEl) metaEl.textContent = currentUser.state + ' · ' + currentUser.acres + ' Acres';
...
// ══════════════════════════════════════════════════
// MODAL  — FIX: use DOMContentLoaded to attach listener
// ══════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════
// AI PRICE ADVISOR — FIX: build system prompt at call time (not module level)
// ══════════════════════════════════════════════════
let advisorHistory = [];
...
async function sendAdvisor() {
  ...
  if (!text) return;
  input.value  = '';
  document.getElementById('advisor-suggestions')?.style && (document.getElementById('advisor-suggestions').style.display = 'none');
  ...
  // Build system prompt at call time so currentUser is populated
  const user   = currentUser || { name: 'farmer', crop: 'crops', state: 'Nigeria' };
...
Farmer: ${user.name}, grows ${user.crop} in ${user.state}, ${user.acres || '?'} acres.
...
function ussdReply() {
  ...
  if (USSD_FLOWS[next] !== undefined) {
    if (USSD_FLOWS[next] === null) {
      ussdState = null;
      _showUssd('Session ended.\n\nDial *347*55# to start again.');
    } else {
      ussdState = next;
    ...
    <div class="mkt-listing">
      ...
      <div class="mkt-listing-footer">
        ...
        <div class="mkt-actions">
          <button class="btn btn-outline" style="font-size:12px;padding:6px 12px" onclick="showToast('💬 Message sent to coop!')">Message</button>
...
window.addEventListener('beforeinstallprompt', e => {
  e.preventDefault();
  _installPrompt = e;
  setTimeout(() => { if (currentUser) _showInstallBanner(); }, 5000);
});
...
function _showInstallBanner() {
  ...
  d.innerHTML = `<div style="position:fixed;bottom:calc(var(--bottom-nav-h,60px) + 14px);left:14px;right:14px;background:#0F1F0F;color:#fff;border-radius:14px;padding:13px 16px;display:flex;align-items:center;gap:12px;box-shadow:0 8px 32px rgba(0,0,0,0.35);z-index:500;font-family:'Outfit',sans-serif;border:1px solid rgba(123,198,123,0.25);">
    ...
    <div style="flex:1;min-width:0"><div style="font-weight:600;font-size:13px">Install AgroLink</div><div style="font-size:11px;color:rgba(255,255,255,0.45)">Works offline · Home screen</div></div>
    <button id="install-btn" style="background:#2D6A2D;color:#fff;border:none;border-radius:8px;padding:7px 13px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Outfit',sans-serif;white-space:nowrap;flex-shrink:0">Install</button>
    <button id="install-dismiss" style="background:t
